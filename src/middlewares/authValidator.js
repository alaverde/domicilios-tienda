...
const mobilePhoneValidator = body("mobile_phone")
  .notEmpty()
  .withMessage("mobile_phone can not be empty")
  .bail()
  .matches(/^3[0-9]{9}$/)
...
const marketNameValidator = body("market_name")
  .if((value, {
    req
  }) => req.body.user_type === "shopkeeper")
  .notEmpty()
  .withMessage("You must provide a market_name")
  .bail()
  .matches(/^[a-zA-ZÀ-ú]+(([',. -][a-zA-ZÀ-ú])?[a-zA-ZÀ-ú]*)*$/)
...
const serviceCapacityValidator = body("service_capacity")
  ...
  .notEmpty()
  .withMessage("You must provide a service_capacity")
  .bail()
  .isInt()
